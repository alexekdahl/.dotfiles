...
if vim.g.loaded_tmux_navigator then
  return
end
vim.g.loaded_tmux_navigator = true
...
local function tmux_navigate(direction)
  ...
  if not tmux or tmux == "" then
    ...
    if direction == "p" then
      -- previous: just use Vim's "previous window"
      vim_navigate("p")
    else
  ...
  -- special case: previous pane
  if direction == "p" then
    local pane = os.getenv("TMUX_PANE")
    if pane and pane ~= "" then
      tmux_command(string.format("select-pane -t %q -l", pane))
    else
      -- fallback: no TMUX_PANE, just use -l
      tmux_command("select-pane -l")
    end
    return
  end
...
vim.api.nvim_create_user_command("TmuxNavigateLeft", function() tmux_navigate("h") end, {})
vim.api.nvim_create_user_command("TmuxNavigateDown", function() tmux_navigate("j") end, {})
vim.api.nvim_create_user_command("TmuxNavigateUp", function() tmux_navigate("k") end, {})
vim.api.nvim_create_user_command("TmuxNavigateRight", function() tmux_navigate("l") end, {})
vim.api.nvim_create_user_command("TmuxNavigatePrevious", function() tmux_navigate("p") end, {})
